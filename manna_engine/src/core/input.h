...
//using windows key codes internally. They also line up with ascii nicely
typedef enum keys {
...
//TODO do differently
MANNA_API b8 is_key_down(keys key);
MANNA_API b8 is_key_up(keys key);
MANNA_API b8 was_key_down(keys key);
MANNA_API b8 was_key_up(keys key);
