...
void create_vulkan_image(
    ...
    image_create_info.arrayLayers = 1; //TODO: support layering
    ...
    if (memory_type == -1) {
        LOG_ERROR("Required memory type not found, image invalid");
    }
    ...
    //bind the memory
    VK_CHECK(vkBindImageMemory(context->device.logical_device, out_image->handle, out_image->memory, 0)); //TODO: configurable offset

    if (create_view) {
        out_image->view = 0;
        create_vulkan_image_view(context, format, out_image, view_aspect_flags);
    }
}
...
void create_vulkan_image_view(vulkan_context *context, VkFormat format, vulkan_image *image, VkImageAspectFlags aspect_flags) {
    ...
    view_create_info.viewType = VK_IMAGE_VIEW_TYPE_2D; //TODO: configurable
